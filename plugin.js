...
var Plugin = Base.extend({
  ...
  getDefaults: function() {
    ...
    return {
      ...
      */
      ...
      /**
      * @cfg {Boolean} [orientable=false]
      * Make the plugin orientable, the navigation window which is the container, will be adapted to the width of the device
      * (considering the orientation), if the layout is fluid (using for example Ti.UI.FILL) it will resize according to
      * the orientation, otherwise must be used specific tags *portrait* and *landscape* in the layout file
      */
      orientable: false,
  ...
  */
  ...
  /**
  * @method onBeforeShow
  * Called just before the window is shown, use this to populate the plugin with data when is activated
  * @deferred
  * @protected
  */
  ...
  /**
  * @method onBeforeHide
  * Called just before the window is hidden, use this to shut down the view (for example pausing a video) when switching
  * to another plugin
  * @deferred
  * @protected
  */
  ...
  */
  getWindowLayout: function(opts) {
    ...
    // if orientable, specify orientation modes
    if (options.orientable) {
      windowLayout.orientationModes = [
  ...
  */
  createNavigation: function() {
    ...
    that._navigation = new Navigation(that._window, {
      orientable: that._options.orientable
    });
